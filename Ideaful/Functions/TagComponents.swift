...
public struct TagInput: View {
    ...
    public init(selectedTags: Binding<[Tag]>, tagManager: TagManager, context: NSManagedObjectContext) {
    ...
    var filteredTags: [Tag] {
        if inputText.isEmpty {
            ...
            return tagManager.allTags.filter { tag in
                !selectedTags.contains(tag) && 
                tag.displayName.lowercased().contains(inputText.lowercased())
            }
        }
    }
...
public struct CreateTagView: View {
    @ObservedObject var tagManager: TagManager
    @Environment(\.presentationMode) var presentationMode
    ...
    public init(tagManager: TagManager) {
        self.tagManager = tagManager
    }
